...
public class Camera2BasicFragment extends Fragment
    ...
    /**
     * Camera state:
     * 0：显示预览 Showing camera preview.
     * 1：等待上锁（拍照片前将预览锁上保证图像不再变化）Waiting for the focus to be locked.
     * 2：准备1（对焦、曝光等）Waiting for the exposure to be precapture state.
     * 3：准备2（闪光灯等操作）Waiting for the exposure state to be something other than precapture.
     * 4：已经获取照片 Picture was taken.
     */
    ...
    private final TextureView.SurfaceTextureListener mSurfaceTextureListener
        ...
        @Override   // SurfaceTexture就绪后执行回调
        public void onSurfaceTextureAvailable(SurfaceTexture texture, int width, int height) {
            openCamera(width, height);
        }

        @Override   // 预览方向改变时，执行转换操作
        ...
        private void process(CaptureResult result) {
            switch (mState) {
                ...
                case STATE_WAITING_PRECAPTURE: {
                    // 等待预处理状态, CONTROL_AE_STATE can be null on some devices
                    Integer aeState = result.get(CaptureResult.CONTROL_AE_STATE);
                    if (aeState == null ||
